...
GENERATED vs hand-written: `wplan-researcher.md` is generated — it is authored
as a document-level `agent` block in `docs/wskills/wplan/wskill.wcl` and
regenerated by `just skills-install`; edit it there, never here. The wbuild-*
and wissue-recon files are hand-written until those skills become wskills.
