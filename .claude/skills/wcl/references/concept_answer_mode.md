...
_The `@answerable` decorator turns question blocks into a guided interview: `wcl answer` and `wcl wdoc serve --answer` walk the pending ones and write the answers back._
...
Two front ends consume it: `wcl answer <file>` in the terminal (arrow-key menus on a TTY, numbered line input otherwise, `--list` / `--id` for scripts and agents) and `wcl wdoc serve --answer` in the browser (a questionnaire drawer over the served site). Both write one question at a time through the validating edit pipeline, so an interrupted session loses nothing.
