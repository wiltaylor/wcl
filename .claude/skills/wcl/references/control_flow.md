...
`try body catch name => handler` evaluates the body; if evaluation fails — a builtin error, an `error()` call, a cycle, or an error propagated from a referenced field — the rendered error message binds to the catch name (a `utf8`, scoped to the handler) and the handler's value is the result. Both sides accept a `{ … }` block.
...
> [!NOTE]
> **Catches everything**
> `try` makes any evaluation failure recoverable, including cycles and upstream field errors — use it at the boundary where a fallback is meaningful, not to paper over schema mistakes (`wcl check` still reports those).
