...
`try body catch name => handler` evaluates the body; if evaluation fails — a builtin error, an
`error()` call, a cycle, or a propagated field error — the rendered message binds to the catch
name (a `utf8`) and the handler's value is the result. Both sides accept a `{ ... }` block.
