...
Every WAD renders the same twelve chapters, so a reader always knows where a fact lives. When
filing data, pick the view first:
...
Freeform topics that fit no family become `article` blocks, filed under a view with
`section = :overview` (etc.).


**Documentation is an output, not a system.** A docs site, book, or skill folder is never a
`system`/`container` in view 4 — it files as a `doc_item` in the documentation view, recording
where it's authored, what builds it, where it's hosted, and which personas it serves.
