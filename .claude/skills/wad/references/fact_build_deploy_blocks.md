...
A `repo_path` that is really another repository says so: `repo` for one of OUR repos pulled in
as a submodule/subtree (the structure diagram links to its page), `external_url` for a
submodule outside our control (drawn dashed). This covers monorepos, multi-repo estates, and
vendored trees with one vocabulary.


Who populates: pipelines and releases are ideal extractor territory (parse the real CI config
/ the git tags — see the reference WAD's `extract_ci.py` and `extract_releases.py`);
repositories start from `extract_repo.py` and get hand-written descriptions.
