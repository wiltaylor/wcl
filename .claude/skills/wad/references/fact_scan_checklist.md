...
| externals | config files, .env.example, outbound URLs and hostnames in code, SDK/client dependencies — a dependency is EVIDENCE of an external, never the external itself: the Stripe SDK points at the Stripe service; SDL2 points at nothing | external_system stubs + relation edges for the remote systems the running code talks to across a boundary. Classify first: a linked/vendored library or framework is the owning container's `technology` (no external, no relation); anything that hosts/builds/ships the system (cloud, CI, CDN, git host, package registry) is an infra_node. If nothing sits on the other end of a network boundary at runtime, emit nothing — a self-contained system honestly has no externals | For each external: vendor contract owner, support contact, and the security posture — never inferable from code. |
