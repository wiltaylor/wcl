...
Status writes are split by role - the invariant is that \*implementation-side agents never touch or hear about status.wcl\*. The authoritative who-writes-which-transition table is the states fact. The reviewer must be a strong model - never route review to the same cheap model that implemented.
