...
Authoring rules: lowest meaningful level only (roll-up derives every higher diagram);
endpoints must resolve to declared ids **written bare** (`source = shop_api`, never
`source = "shop_api"` — quoted endpoints break the roll-up on older binaries); one relation
per meaningful edge — parallel relations between the same rolled pair merge into one labelled
edge. Who populates: hand for business/persona edges; extractors for dependency edges a build
