...
generates it. Three zones: `schema/` is what this topic adds to the data model, `data/` is
the content you write, `wdoc/` is the projection entry points. Never hand-edit `out/`.
...
What is NOT in the tree is as important: the base schema, the two optional-view schemas and
the shared page components ship **inside the `wcl` binary** and arrive by import, exactly as
`import <wdoc.wcl>` already does. The root document says `import <wskill.wcl>`; `wdoc/book/main.wcl`
is two lines — the topic's model plus `import <wskill/book.wcl>`. They used to be copied into
every folder, which is how four wskills came to carry ~56 files nobody was meant to edit.
