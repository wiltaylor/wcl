...
//! [`unions::validate_union`](super::unions) (foundation pass); the
//! dispatcher's defensive `VariantAmbiguous` arm should be unreachable
//! in normal use. What to do with the match — rewriting a bare record
//! into the variant it matched — is [`inhabit`](super::inhabit)'s half
//! of the job.
...
pub(crate) fn block_to_variant<'a>(
    doc: &'a Document,
    block: &Block<'a>,
...
pub(crate) fn decorator_to_variant<'a>(
    doc: &'a Document,
    positional: &[Value],
...
/// positionally.
pub(crate) fn table_row_to_variant<'a>(
    doc: &'a Document,
    row_block: &Block<'a>,
    union_decl: UnionDecl<'a>,
) -> Result<Value, EvalError> {
