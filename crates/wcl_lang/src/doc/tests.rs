...
// ── `@contextual` and the expander callback ──────────────────────────
//
// A `@contextual` block's placement is decided by context, not by kind,
// and its children are generated by the host's `Expander`. These tests
// use a miniature host vocabulary — a `loop` that repeats its body once
// per element of `each` — so nothing here depends on wdoc's.

/// A test host's expander: `loop` repeats its body once per element of
/// `each`, binding the element to `it`. Every other kind generates
/// nothing, exactly as a real host's would.
struct LoopExpander;

impl crate::Expander for LoopExpander {
    fn expand<'a>(&self, block: &Block<'a>) -> Vec<Block<'a>> {
        if block.kind() != "loop" {
            return Vec::new();
        }
        let Some(Value::List(items)) = block.field("each").and_then(|f| f.value().ok().cloned())
        else {
            return Vec::new();
        };
        let sets = std::sync::Arc::unwrap_or_clone(items)
            .into_iter()
            .map(|el| std::sync::Arc::new(vec![("it".to_string(), el)]))
            .collect();
        block
            .expand_bodies(block, sets)
            .into_iter()
            .flatten()
            .collect()
    }
...
const CONTEXTUAL_SRC: &str = r#"
@document("d") type D { @children("deck") decks: list<Deck> }
@block("deck") type Deck {
  @inline(0) name: identifier
  @children("card") cards: list<Card>
}
@block("card") type Card { title: utf8 }
@block("loop") @contextual type Loop {
...
fn open_contextual(with_expander: bool) -> Document {
    let mut env = Environment::new();
    if with_expander {
        env.set_expander(std::sync::Arc::new(LoopExpander));
    }
    Document::open_with(CONTEXTUAL_SRC, "test", &env).expect("open")
}

#[test]
fn contextual_block_is_legal_wherever_children_are_allowed() {
    // `loop` is not among `deck`'s declared child kinds, yet placing one
    // there is legal: `@contextual` — not a hardcoded kind list — is what
    // exempts it.
    let doc = open_contextual(true);
    assert!(
        doc.schema_errors().is_empty(),
        "a @contextual block is legal inside a block that allows children: {:?}",
        doc.schema_errors()
    );
}

#[test]
fn contextual_block_body_is_not_recursed_into() {
    // A body only has meaning once expanded with bindings, so the child
    // walk does not validate it in place — `bogus` would otherwise be an
    // UnknownField on `card`.
    let src = CONTEXTUAL_SRC.replace("card { title = it }", "card { bogus = 1 }");
    let mut env = Environment::new();
    env.set_expander(std::sync::Arc::new(LoopExpander));
    let doc = Document::open_with(&src, "test", &env).expect("open");
    assert!(
        doc.schema_errors().is_empty(),
        "a @contextual body is validated at expansion, not in place: {:?}",
        doc.schema_errors()
    );
}

#[test]
fn expander_generated_children_reach_the_children_projection() {
    let doc = open_contextual(true);
    ...
    let titles: Vec<String> = cards
        .children()
        .map(
            |c| match c.as_block().unwrap().field("title").unwrap().value() {
                Ok(Value::Utf8(s)) => s.clone(),
    ...
    assert_eq!(
        titles,
        vec!["literal", "a", "b"],
        "the literal child and the expander's two generated ones share the slot"
    );
}

#[test]
fn missing_expander_errors_when_generated_children_are_demanded() {
    // Opening the document is fine — parse and format never demand an
    // expansion.
    let doc = open_contextual(false);
    let deck = doc.block("deck").expect("deck block");
    // Demanding the slot is where it fails, and the error names the kind
    // whose expander is missing.
    let err = deck
        .typed_field("cards")
        .expect("cards slot")
        .value()
        .expect_err("a missing expander is a hard error");
    assert!(
        matches!(&err, EvalError::MissingExpander { kind, .. } if kind == "loop"),
        "expected MissingExpander naming 'loop', got {err:?}"
    );
}
...
fn missing_expander_does_not_affect_blocks_that_generate_nothing() {
    ...
    let src = CONTEXTUAL_SRC.replace(
        "  loop { each = [\"a\", \"b\"]\n    card { title = it }\n  }\n",
        "",
    );
    assert!(!src.contains("loop {"), "the loop instance was removed");
    let doc = Document::open_with(&src, "test", &Environment::new()).expect("open");
    let deck = doc.block("deck").expect("deck block");
    assert_eq!(
        deck.typed_field("cards").expect("cards slot").len(),
        Some(1),
        "an ordinary child projection needs no expander"
    );
    assert!(
        !deck.is_contextual(),
        "a plain @block kind is not contextual"
    );
    assert!(
        deck.expand_children()
            .expect("no expansion demanded")
            .is_empty(),
