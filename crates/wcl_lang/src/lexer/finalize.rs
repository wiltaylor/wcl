//! Turning a carved-up numeric literal into a typed number.
//!
//! The second half of number lexing. [`numbers`](super::numbers) scans
//! the text and hands over the pieces — sign, base prefix, digit body,
//! optional fraction, optional exponent, optional suffix — as a
//! [`ParsedNumber`]; [`finalize`] decides what type that is and whether
//! the value fits it. Every range check and base conversion in the
//! language happens here, so "does `300u8` fit?" has exactly one answer.
//!
//! An unrecognised suffix is not an error: it becomes a
//! [`NumberLit::Unit`], to be resolved later against the declared type's
//! `@unit` decorators.
