...
pub(crate) fn layout_items_to_value(items: &[LayoutItem]) -> Value {
    Value::List(items.iter().map(layout_item_to_value).collect())
}

fn layout_item_to_value(item: &LayoutItem) -> Value {
    match item {
        LayoutItem::SplitGroup(group) => split_group_to_value(group),
        LayoutItem::Content(block) => content_block_to_value(block),
    }
}

fn split_group_to_value(group: &SplitGroup) -> Value {
    ...
    map.insert(
        "splits".to_string(),
        Value::List(group.splits.iter().map(split_to_value).collect()),
    );
    Value::Map(map)
}

fn split_to_value(split: &Split) -> Value {
    let mut map = IndexMap::new();
    ...
    map.insert(
        "children".to_string(),
        layout_items_to_value(&split.children),
    );
    Value::Map(map)
}
