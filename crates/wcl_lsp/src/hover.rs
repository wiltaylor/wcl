    ...
    for scope in analysis.scopes.all_scopes() {
        if let Some(entry) = scope.entries.get(&ident.name) {
            ...
            let kind = match entry.kind {
                wcl_eval::ScopeEntryKind::LetBinding => "let",
...
fn extract_doc_comments(trivia: &wcl_core::trivia::Trivia) -> String {
    trivia
        .comments
        .iter()
        ...
        let md = MacroDef {
            ...
            params: vec![
                MacroParam {
                    ...
                    type_constraint: Some(TypeExpr::String(wcl_core::Span::dummy())),
                    default: None,
                    span: wcl_core::Span::dummy(),
                },
                MacroParam {
                    ...
                    type_constraint: Some(TypeExpr::Int(wcl_core::Span::dummy())),
                    default: None,
                    span: wcl_core::Span::dummy(),
                },
            ],
