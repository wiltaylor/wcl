...
#[derive(Default)]
struct Decls {
    /// Union declarations, by name.
    unions: BTreeMap<String, UnionDecl>,
    /// Record declarations, by name.
    records: BTreeMap<String, TypeDecl>,
    /// Symbol-set declarations, by name.
...
/// Parse every stdlib source and collect the declarations the IR is
/// generated from.
...
impl Decls {
    /// Record that a name is declared, so collisions can be detected.
    ...
    /// Whether any source declares this name.
    ...
    /// Register a symbol-set declaration.
...
/// Which sort of declaration a name refers to.
...
/// Collect every declared type name a type reference mentions, so the
/// generator knows what it must also emit.
...
/// Generate the Rust source for the whole content IR.
...
/// Generate a Rust enum for one union.
...
/// Generate a Rust struct for one record.
...
/// Generate a Rust enum for one symbol set.
...
/// Turn a WCL name into a valid Rust identifier, escaping keywords.
...
/// Convert a snake_case WCL name to CamelCase for a Rust type.
...
/// Emit a doc comment for a generated item: the declaration's `@doc`
/// text when it has one, else `fallback`. Every call site supplies a
/// non-empty fallback, because the generated file is compiled under the
/// workspace documentation lints and an undocumented item there cannot
/// be fixed by editing the output.
...
/// A declaration's `@doc` text, to carry into the generated item's
/// own doc comment.
