...
//! *intent* half — rationale and implementation instructions — is left as
//! explicit TODOs for a human or agent to fill; the tool never pretends to
//! know why a change happened.
...
pub(crate) fn run_spec(
    ...
    println!(
        "next: add `import \"./{spec_id}.wcl\"` to the specs data hub, fill in the TODO rationale/instructions, and keep `wcl check` green."
    );
    EXIT_OK
}
...
fn render_spec(id: &str, title: &str, sha: &str, changes: &[diff::Change]) -> String {
    ...
    out.push_str(
        "// Change-spec skeleton generated by `wcl wad spec` — the change list is\n\
         // mechanical fact; fill in the summary and the rationale/instructions body,\n\
         // then move `status` as work proceeds (:planning → :in_progress → :complete).\n\
         namespace wcl.wad\n\n",
    );
    ...
    // The bar for the filled-in body: enough detail that an implementing
    // agent needs nothing else — context, ordered instructions naming the
    // files to touch, and checkable acceptance criteria.
    out.push_str(
        "\n  body {\n    p \"TODO(rationale): why these changes were made.\"\n    p \"TODO(instructions): what an implementing agent must do, in order — name the files/modules to touch.\"\n    p \"TODO(acceptance): the checkable criteria that prove the work is done (commands, tests, rendered output).\"\n  }\n}\n",
    );
    out
}
