...
impl DecoratorSchemaRegistry {
    ...
    pub fn collect(&mut self, doc: &Document, diagnostics: &mut DiagnosticBag) {
        for item in &doc.items {
            if let DocItem::Body(BodyItem::DecoratorSchema(ds)) = item {
                let name = crate::schema::schema::string_lit_to_string(&ds.name);
    ...
    fn resolve_decorator_schema(&self, ds: &DecoratorSchema) -> ResolvedDecoratorSchema {
        ...
        for field in &ds.fields {
            let required =
                !crate::schema::schema::has_decorator(&field.decorators_before, "optional")
                    && !crate::schema::schema::has_decorator(&field.decorators_after, "optional");
            let default =
                crate::schema::schema::get_decorator_value(&field.decorators_before, "default")
                    .or_else(|| {
                        crate::schema::schema::get_decorator_value(
                            &field.decorators_after,
                            "default",
                        )
                    });
        ...
        ResolvedDecoratorSchema {
            name: crate::schema::schema::string_lit_to_string(&ds.name),
            ...
            for (i, arg) in decorator.args.iter().enumerate() {
                ...
                if let Some(pname) = param_name {
                    if let Some(param) = schema.params.iter().find(|p| p.name == pname) {
                        if let Some(value) = crate::schema::schema::expr_to_value(expr) {
                            if !crate::schema::types::check_type(&value, &param.type_expr) {
                                diagnostics.error_with_code(
                                    format!(
                                        "decorator @{}: parameter '{}' expects type {}, got {}",
                                        name,
                                        pname,
                                        crate::schema::types::type_name(&param.type_expr),
                                        value.type_name()
                                    ),
                                    decorator.span,
                                    "E063",
