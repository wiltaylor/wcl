                ...
                if let Some(shadowed_span) = self.scopes.check_shadowing(scope_id, &lb.name.name) {
                    if !has_allow_decorator(&lb.decorators, "shadowing") {
                        self.diagnostics.add(
                            crate::lang::diagnostic::Diagnostic::warning(
                                format!(
            ...
            Expr::Ident(ident) => {
                ...
                if name == "is_imported" {
                    ...
                    let result = if let Some(base) = &self.base_dir {
                        let resolved = crate::eval::imports::normalize_path(&base.join(&path_str));
...
mod tests {
    ...
    fn count_warnings_with_code(ev: &Evaluator, code: &str) -> usize {
        ev.diagnostics()
            ...
            .iter()
            .filter(|d| {
                d.severity == crate::lang::diagnostic::Severity::Warning
