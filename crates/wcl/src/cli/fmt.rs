...
impl<'a> Formatter<'a> {
            ...
            DocItem::Use(use_decl) => {
                ...
                if use_decl.targets.len() == 1 {
                    ...
                    if let Some(ref alias) = t.alias {
                        self.output.push_str(&format!(" -> {}", alias.name));
                    }
                } else {
                    self.output.push_str(&format!("use {}::{{", ns_path));
                    for (i, t) in use_decl.targets.iter().enumerate() {
                        if i > 0 {
                            self.output.push_str(", ");
                        }
                        self.output.push_str(&t.name.name);
                        if let Some(ref alias) = t.alias {
                            self.output.push_str(&format!(" -> {}", alias.name));
                        }
                    }
    ...
    fn format_string_lit(&mut self, s: &StringLit) {
        if let Some(ref info) = s.heredoc {
            ...
            self.format_quoted_string(s);
        }
    }

    fn format_quoted_string(&mut self, s: &StringLit) {
        self.output.push('"');
        for part in &s.parts {
            match part {
    ...
    fn format_heredoc(&mut self, s: &StringLit, info: &HeredocInfo) {
        ...
        self.output.push('\n');
        // Emit closing delimiter at current indent level
        if info.indented {
            self.output.push_str(&indent_str);
        }
        self.output.push_str(tag);
    ...
    fn format_type_expr(&mut self, te: &TypeExpr) {
        match te {
            ...
            TypeExpr::I8(_) => self.output.push_str("i8"),
            TypeExpr::U8(_) => self.output.push_str("u8"),
            TypeExpr::I16(_) => self.output.push_str("i16"),
            TypeExpr::U16(_) => self.output.push_str("u16"),
            TypeExpr::I32(_) => self.output.push_str("i32"),
            TypeExpr::U32(_) => self.output.push_str("u32"),
            TypeExpr::I64(_) => self.output.push_str("i64"),
            TypeExpr::U64(_) => self.output.push_str("u64"),
            TypeExpr::I128(_) => self.output.push_str("i128"),
            TypeExpr::U128(_) => self.output.push_str("u128"),
            TypeExpr::F32(_) => self.output.push_str("f32"),
            TypeExpr::F64(_) => self.output.push_str("f64"),
            TypeExpr::Date(_) => self.output.push_str("date"),
            TypeExpr::Duration(_) => self.output.push_str("duration"),
