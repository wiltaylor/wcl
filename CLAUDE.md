...
| review comments + fleeting notes (`comments.wcl` sidecar beside each wskill / root doc — page+locator keyed, placed client-side, watcher-ignored so no rebuild; `wcl wdoc comments` + `serve --comment`; fleeting notes in `.wdoc-fleeting.json`) | `src/comments.rs`, dev server `crates/wcl/src/serve.rs` | `comment.wcl` | `comments.wcl` |
