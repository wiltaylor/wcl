...
| the class lint (after a full build, diff the class names the rendered HTML carries against the ones the rules select, both directions, at warning level — plus the `//`-in-a-declaration build error in `render/css.rs`. Two structural exemptions carry it, and without them it cried wolf 178 times with 0 true positives: generator vocabularies (`tok-*` / `language-*`, each prefix named by its generator) and library rules (wdoc's stylesheet ships the whole vocabulary, so only rules the DOCUMENT authors can be called unused). A deliberate hook is declared by an empty `class "name" {}`, which emits nothing and silences the name. Runs over the union of a document's sites, never one — so `--site` and the dev server's targeted rebuild skip it. A renderer that resolves a class instead of emitting it (wireframe, terminal) reports the use itself.) | `src/css_lint.rs`, `write_html_page` + `site_css` in `src/build.rs` | — | `wdoc_styling` |
