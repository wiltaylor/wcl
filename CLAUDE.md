...
- **`wcl` CLI** — `parse` / `check` / `eval`+`get` / `set` / `fmt` / `diff` (WCL-aware entity/field document diff → JSON) / `repl` / `lsp` / `wdoc {build,serve,pdf,markdown,skill}`. → see `docs/pages/reference/ref/cli.wcl` and `wcl --help`.
