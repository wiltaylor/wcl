...
| review comments (`comments.wcl` sidecar beside each wskill / root doc — page+locator keyed, placed client-side, watcher-ignored so no rebuild; `wcl wdoc comments` + `serve --comment`) | `src/comments.rs`, dev server `crates/wcl/src/serve.rs` | `comment.wcl` | `comments.wcl` |
| review handshake (`wcl wdoc review <file>` — the agent's blocking wait: registers as waiting, blocks until the reviewer clicks "✅ Send to agent" in the comment toolbar, then prints the comments like `comments`; calling it again re-shows the toolbar banner — the "agent finished, rebuild & review" loop. File-based markers under `<tmp>/wcl-wdoc-review/<hash-of-canonical-root>/` (`serve`/`agent`/`ready`, round-keyed) — no HTTP client, no port discovery; serve writes the live-server marker on startup when `--comment` and clears it on Ctrl-C. Endpoints `GET /__wdoc_review/status` (long-poll on the `agent` marker) + `POST /__wdoc_review/ready`, both comment-mode only) | `crates/wcl_wdoc/src/review.rs` (`Handshake` markers), `crates/wcl/src/main.rs` (`run_review`), dev server + toolbar JS in `crates/wcl/src/serve.rs` | — | — |
| WYSIWYG editor (`serve --edit`, composes with `--comment`) — select a rendered block to edit its fields in a side-panel form / double-click text to edit inline / add·move·delete blocks, plus an object editor that browses schema `@block`/`@table` data objects (filtered by namespace, sorted alphabetically) and edits each as **raw WCL text** in a textarea — an existing object's source is byte-spliced back (formatting preserved, no reformat); a new object starts from a schema template and is appended to its target file. Block/field edits write real `.wcl` source via the `wcl set` pipeline (`parse_for_edit`→mutate by byte span→`format::to_source`→`verify_reparses`→`write_atomic`); object text edits byte-splice/append; the dev server does **not** auto-rebuild — a manual rebuild (Enter in the console, or the comment toolbar's Rebuild button → `POST /__wdoc_rebuild`) applies the change and reloads. The Rebuild button sends the current page's `page_file`; the server scopes the rebuild to that page's owning sub-site (via `subsite_for_page` → builds only that wskill into its output subdir) instead of the whole top-level site, and the endpoint **blocks until the build finishes** so the client shows a running spinner then a done toast (carried across the reload in `sessionStorage`). Console Enter rebuilds the whole site. Block anchoring stamps `data-wcl-span`/`data-wcl-file` (gated on `InlinePatterns::anchor_mode()`, edit-mode only); new-object placement via the `@wdoc.file(path, folder?)` decorator (UI override allowed). Edits reopen the doc via `wcl_wdoc::open_doc_for_edit`; constraint errors roll back (baseline-diff of `schema_errors`). When `--edit` is pointed at a **top-level** site, each request carries the current page's `data-wcl-page-file`; the server resolves the owning `include`d sub-site (e.g. a wskill book) via `wcl_wdoc::doc_entry_for_page` (`include::entry_for_page`) and introspects *that* document, so a wskill page's own kinds (concept/entity/fact/…) show up rather than the root's. Object kinds carry a `namespace` and are sorted; instances sorted by label. An **`edit_object` block** (stdlib `edit_object.wcl`, special-cased in `render_block` → emitted **only when `patterns.edit_mode()`**, skipped by `anchor_block` so it isn't itself selectable; `[]` WCL `lower` ⇒ nothing in build/markdown/pdf) renders an "Edit this …" button carrying `data-wcl-edit-kind`/`-target`; the client resolves the target against `/__wdoc_objects` (matched on the instance's label/id) and opens the object text editor for that exact instance. Wired into every concept/entity/fact/process via the wskill component templates (`docs/wskills/*/wdoc/component/` + the `wskill` scaffold). | AST helpers `crates/wcl_lang/src/edit.rs`; server logic + JS client `crates/wcl/src/edit.rs` (+ `edit_client.js`), routes/anchors in `crates/wcl/src/serve.rs` + `src/render/html.rs` (`anchor_block`, `render_edit_object_button`) | `file_placement.wcl`, `edit_object.wcl` | — |
| Source editor + preview (`serve --edit`) — a "⌨ Source" toolbar overlay: file tree (`GET /__wdoc_files`, scoped to the page's owning sub-site), whole-file editing of any `.wcl` (`GET/POST /__wdoc_file`, content-etag conflict detection, saves through the validating `commit` pipeline), the shared `WclEditor` component (syntect-highlighted `<pre>` backdrop under a transparent textarea — native undo/IME; served at `/__wdoc_editor.js`, also replaces the object editor's textarea), live dry-run diagnostics (`POST /__wdoc_check`: buffer syntax with exact line/col + document schema errors *introduced* by the buffer, computed via `open_doc_for_edit_with_overlay` over `wcl_lang::overlay_loader`), `POST /__wdoc_format` (`wcl fmt` core), `POST /__wdoc_highlight` (public `wcl_wdoc::highlight_code`). **Preview without saving**: `POST /__wdoc_preview` renders the current page with unsaved buffers overlaid into a scratch TempDir (`BuildOptions.overlay` + `page_filter`; first preview warms with a full sub-site build, then ~sub-second targeted single-page renders); `GET /__wdoc_preview/{*path}` serves it with **no** reload/edit scripts; shown in an iframe pane. | `crates/wcl/src/{preview.rs,editor_client.js}`, endpoints in `src/edit.rs`, routes in `src/serve.rs`; overlay/page_filter in `crates/wcl_wdoc/src/build.rs` | — | — |
| Answer mode (`serve --answer`, composes with `--comment`/`--edit`) — a questionnaire drawer over pending `@answerable` question blocks (`import <answer.wcl>` declares the decorator mapping prompt/response/status roles + pending/resolved/skipped symbols onto the user's own question schema, plus a ready-made `option` child block): radio group for `:single_select`, checkboxes for `:multi_select`, free text always present (options never constrain), Skip only when a skipped symbol is declared. `GET /__wdoc_answers` lists pending questions (sub-site-resolved via `entry_for`), `POST /__wdoc_answer` re-discovers by file+span (stale spans get a clean error) and writes response+status through the same validating `commit` pipeline — one question per write, per-answer durability. The shared core also drives `wcl answer <file>` (`--list` JSON / `--id` non-interactive / interactive arrow-key menus — raw mode by shelling to `stty`, since the workspace forbids `unsafe`; numbered `:skip`/`:later`/`:quit` line input otherwise; re-discovers between answers because each write reformats the file and shifts spans). Fixture: `examples/answer/plan.wcl`. | `crates/wcl/src/{answer.rs,answer_tui.rs,answer_client.js}`, routes in `src/serve.rs` | `answer.wcl` (top-level import, not in the wdoc prelude) | wskill concept `answer_mode` |
