...
- `crates/wcl_wdoc` — library crate driving the `wcl wdoc` subcommands: `wcl_wdoc::build(file, out_dir)` renders pages to HTML, `wcl_wdoc::serve(file, out, addr)` runs the watch+axum dev server. Ships a bundled `wdoc.wcl` schema with **fundamental blocks only**: `page`, `text` + `span`, `column` (CSS-percent grid), `diagram` + shapes (`rect`, `circle`, `line`, `label`, `polygon`, `container` — SVG `<g>` group; supports `layout = :none|:grid` with `columns`/`cell_width`/`cell_height`/`gap`, plus declared `width`/`height` for non-laid-out parents). Every shape (incl. `container`) takes per-edge `anchor_left`/`anchor_right`/`anchor_top`/`anchor_bottom` fields that resolve against the parent's box — opposite anchors stretch, single anchors pin. Sizing cascades: a container resized by its parent's grid passes the new size to its anchored children, and `class` (site-level style bundle). These are the IR layer the HTML renderer (and future SVG/other targets) speak; higher-level blocks (headings, lists, callouts, …) will land in a follow-up slice that lowers them to these. Each IR block carries an optional `class: list<utf8>?` field; site-level `class` blocks emit `.name { ... }` rules into a `<style>` block in `<head>` and matching `class="..."` attributes on elements — the browser handles cascade. `serve` renders into a tempdir (or `--out`) by calling `build()`, watches the source dir via `notify` and rebuilds on every `.wcl` change; axum routes `/` and `/<name>[.html]` are static-file reads off that directory. Build errors are logged to stderr; the last successful render keeps serving.
