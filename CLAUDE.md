...
`docs-relative-urls` greps the sites `docs-build` just wrote for a root-absolute
`href="/…"` / `src="/…"`. A build tree is relocatable — the book is deployed under
`/reference/`, a directory the build was never told about — and an absolute URL breaks
that. The Rust tests resolve every local target over a fixture book
(`crates/wcl_wdoc/tests/build.rs`, `crates/wcl/tests/wdoc.rs`); this part stays a grep, and
only exists to catch an absolute link typed into a real page.
